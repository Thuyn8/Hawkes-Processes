...
thres <- 30
S2_thres <- sapply(S2[, !(colnames(S2) %in% c("D6W5", "D19W10"))], 
...
# creating L2 and L20 data sets including spike times for each fly
## sugar 2% (s2)
...
# estimate mu_0, alpha, beta using LH 
## s2
...
## obtain intensity rates for each data set
...
control <- sapply(control, function(i){as.integer(i>=thres_dist)}) %>% 
            as.data.frame() %>% 
            mutate(Time = 1:nrow(control))
head(control)

npf <- sapply(npf, function(i){as.integer(i>=thres_dist)}) %>% 
        as.data.frame() %>% 
        mutate(Time = 1:nrow(npf))
head(npf)

## Check missing data
sapply(control, function(i){100*mean(is.na(i))}) %>%signif(3)  # T2L,T4L have NAs
sapply(npf, function(i){100*mean(is.na(i))})%>% signif(3)  # T3R has missing
